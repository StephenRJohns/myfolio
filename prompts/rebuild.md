...
const state = {
  ...
  transactions: [],
  performance: {},
  dailyValues: [],            // [{date, value}] — portfolio-level
...
- Zero-balance / closed accounts auto-hide
...
When an account has no daily value history (e.g. recently opened via rollover or beneficiary transfer), MyFolio reconstructs the series by walking captured transactions backwards from today's balance. Requires the user to visit the brokerage Activity page once per session to capture the transaction endpoint.
...
### Local Storage Keys
- `chrome.storage.local`: benchmark price cache (keyed by `mf_benchmark_{ticker}`), load-time samples (`mf_avg_load_ms`)
- `localStorage` (domain-scoped): selected benchmarks (`mf_benchmarks`)
