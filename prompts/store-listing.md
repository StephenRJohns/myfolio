...
WHAT YOU GET
▸ Overview — Total portfolio value, today's change, per-account cards with last-four digits of account number. Click any card to drill into a single account. Hover the Value Over Time or Account Performance Comparison chart to expand it and see a vertical crosshair tooltip with exact values for any date. Click any allocation donut slice or legend row to filter Holdings to that asset class.
▸ Holdings — Full positions table with allocation %, gain/loss $, and gain/loss %. Sortable. Allocation donut chart. Hover any row to expand it for easier reading. Export to CSV.
▸ Transactions — Chronological activity with color-coded type badges. Hover any row to reveal a full detail panel with complete description, account name, and all field values. Export to CSV.
▸ Activity — Cash flows only: deposits, withdrawals, rollovers, and transfers. Shows net contributions and one-click data loading. Hover any row to see the full description and account details. Export to CSV.
